...
void sharpen(const cv::Mat& image, cv::Mat& result) {
	...
	for (int j = 1; j < image.rows - 1; j++) { // for all rows (except first and last)
		...
		for (int i = nchannels; i < (image.cols - 1) * nchannels; i++) {
			...
			*output++ = cv::saturate_cast<uchar>(6 * current[i] - current[i - nchannels] - current[i + nchannels] - previous[i] - next[i]);
		}
	}
...
int main()
{
	...
	cv::Mat image = cv::imread("moon.jpg");
	if (!image.data)
		return 0;
